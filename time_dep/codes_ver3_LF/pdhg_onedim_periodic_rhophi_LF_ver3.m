...
function [phi, error_all, phi_next, phi_bar, rho_next] = pdhg_onedim_periodic_rhophi_LF_ver3(g, phi0, dx, dt, M, H_fn, dH_fn, stepsz_param, rho0, if_forward, if_square)
if if_forward ~= 1
...
% compute H((phi_{a,i+1} - phi_{a,i})/dx) / 2 + H((phi_{a,i} - phi_{a,i-1})/dx) / 2
%       a = k if forward Euler; k+1 if backward Euler
...
% define df for HJ
% f = [f1; f2]
% (f1)_{i,k} = (phi_{i,k+1} - phi_{i,k})/dt + H((phi_{i+1,k} - phi_{i,k})/dx)/2 + H((phi_{i,k} - phi_{i-1,k})/dx)/2 - M(phi_{i+1,k} + phi_{i-1,k} - 2phi_{i,k})/dx
% (f2)_{i} = phi_{i,1} - g_i
...
if if_forward == 1  % forward Euler: using phi^k in H and viscosity term
    ...
    % d(f1)_{i,k} / dphi_{i,k} = -1/dt - dH((phi_{i+1,k} - phi_{i,k})/dx)/(2dx) + dH((phi_{i,k} - phi_{i-1,k})/dx)/(2dx) + 2M/dx
    ind_input_1 = sub2ind([nt, nx], row_output_f1(:), col_output_f1(:));
    val_1 = -1/dt - dH_fn(dphibar_right / dx) /(2*dx) + dH_fn(dphibar_left / dx)/(2*dx) + 2*M/dx;

    % d(f1)_{i,k} / dphi_{i+1,k} = dH((phi_{i+1,k} - phi_{i,k})/dx)/(2dx)- M/dx
    col_output_f1_right = [col_output_f1(:,2:end), col_output_f1(:,1)];
    ind_input_2 = sub2ind([nt, nx], row_output_f1(:), col_output_f1_right(:));
    val_2 = dH_fn(dphibar_right / dx) /(2*dx) - M/dx;
    
    % d(f1)_{i,k} / dphi_{i-1,k} = -dH((phi_{i,k} - phi_{i-1,k})/dx)/(2dx)- M/dx
    col_output_f1_left = [col_output_f1(:,end), col_output_f1(:,1:end-1)];
    ind_input_3 = sub2ind([nt, nx], row_output_f1(:), col_output_f1_left(:));
    val_3 = -dH_fn(dphibar_left / dx) /(2*dx) - M/dx;
    
    % d(f1)_{i,k} / dphi_{i,k+1} = 1/dt
    ind_input_4 = sub2ind([nt, nx], row_output_f1(:) + 1, col_output_f1(:));
