...
function [phi, error_all, phi_next, rho_next, mu_next] = pdhg_onedim_periodic_rho_m_EO_L1_xdep(f_in_H, c_in_H, phi0, rho0, m0, mu0, stepsz_param, if_fwd, g, dx, dt, c_on_rho, if_precondition)
N_maxiter = 1e7; %1e7;
eps = 1e-6;
...
% define Dx and Dt for preconditioning
Dx = -A1 / dt;
Dt = -A2 / dt;
...
D = [Dx; Dt]'*[Dx; Dt];
Aprecondition = [D; Abd];
% Aprecondition_inv = inv(Aprecondition);
...
for i = 1: N_maxiter
    ...
    if if_precondition
        ...
        phi_next = Aprecondition\([D* phi_prev(:) - delta_phi(:); g(:)]);
...
function phi_next = update_phi_preconditioning(vec, phi_prev)
...
for l =1:nt
    v_Fourier(l,:) =  mirt_dctn(vec(l,:));
end
% 0 = tau* v + (Dx + Dt)^T(Dx + Dt)(phi - phi_prev)
% %for each fourier mode, solve the -(cx 2pi xi).^2 * I + ct nalba_t = rhs
