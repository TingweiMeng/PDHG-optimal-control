...
def update_phi_preconditioning(delta_phi, phi_prev, fv, dt):
  ...
  neg_Lap_t_diag = jnp.array([-2/(dt*dt)] * (nt-1) + [-1/(dt*dt)])  # [nt]
  neg_Lap_t_diag_rep = einshape('n->nm', neg_Lap_t_diag, m = nx)  # [nt, nx]
  thomas_b = einshape('n->mn', fv, m = nt) + neg_Lap_t_diag_rep # [nt, nx]
