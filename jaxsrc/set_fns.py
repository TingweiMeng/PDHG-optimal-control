...
def set_up_numerical_L(egno, ndim, ind, fn_coeff_H):
  ...
  # fn_coeff_L is from x_arr, t_arr to [..., ndim]
...
def set_up_example_fns(egno, ndim, numerical_L_ind):
  ...
  if ndim == 2:  # f_i = -cf_i(x,t)^T alp, dim_ctrl = dim_state = ndim, L = |alp|^2/2/cH(x,t), cH(x,t) = 1
    if egno == 1:
    ...
    coeff_fn_H = lambda x_arr, t_arr: jnp.ones_like(x_arr)  # [..., ndim] -> [..., ndim]
    ...
    numerical_L_fn, DL_fn, HL_fn = set_up_numerical_L(egno, ndim, numerical_L_ind, coeff_fn_H)
  ...
  if ndim == 1:
    Functions = namedtuple('Functions', ['f_fn', 'Df_fn', 'Hf_fn', 'numerical_L_fn', 'DL_fn', 'HL_fn', 
                                         'alp_update_fn'])
    fns_dict = Functions(f_fn=f_fn, Df_fn=Df_fn, Hf_fn=Hf_fn,
