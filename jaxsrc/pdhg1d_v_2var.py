...
def update_rho_1d(rho_prev, phi, vp, vm, sigma, dt, dx, epsl, c_on_rho, Hstar_plus_fn, Hstar_minus_fn):
...
def update_v_1d(vp_prev, vm_prev, phi, rho, sigma, dx, c_on_rho, 
               Hstar_plus_prox_fn, Hstar_minus_prox_fn, eps=1e-4):
  '''
  @ parameters:
  ...
  vp_next = Hstar_minus_prox_fn(vp_next_raw, t)  # [nt-1, nx]
  vm_next = Hstar_plus_prox_fn(vm_next_raw, t)  # [nt-1, nx]  
  return vp_next, vm_next

  
def update_primal_1d(phi_prev, rho_prev, c_on_rho, vp_prev, vm_prev, tau, dt, dx, fv, epsl, if_precondition=True):
  eps = 1e-4
...
def update_dual_1d(phi_bar, rho_prev, c_on_rho, vp_prev, vm_prev, sigma, dt, dx, epsl, fns_dict, rho_v_iters=10, eps=1e-7):
  '''
  @ parameters:
